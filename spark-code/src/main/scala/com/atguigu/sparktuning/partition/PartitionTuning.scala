...
object PartitionTuning {
  def main( args: Array[String] ): Unit = {
    val sparkConf = new SparkConf().setAppName("PartitionTuning")
      .set("spark.sql.autoBroadcastJoinThreshold", "10m")//为了演示效果，先禁用了广播join
      .set("spark.default.parallelism", "6")
       .setMaster("local[*]")
      .set("spark.sql.shuffle.partitions", "36")    // 需要shuffle是mapper端写出的partition个数
    val sparkSession: SparkSession = InitUtil.initSparkSession(sparkConf)

    //查询出三张表 并进行join 插入到最终表中
    val saleCourse = sparkSession.sql("select * from sparktuning.sale_course limit ")
